...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          'text-foreground/70 font-medium inline-flex h-full items-center gap-1.5 text-[0.8125rem] leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60',
          className,
        )}
      >
        {icon && icon}
  ...
  const headerButton = () => {
    return (
      <Button
        variant="ghost"
        className={cn(
          'text-foreground/70 rounded-md font-medium -ms-2 px-2 h-7 hover:bg-muted hover:text-foreground data-[state=open]:bg-muted data-[state=open]:text-foreground',
          className,
        )}
