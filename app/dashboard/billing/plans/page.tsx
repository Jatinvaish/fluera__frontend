...
export default function PlansPage() {
  ...
  const calculateDiscount = (cycle: 'quarterly' | 'annual') => {
    const paidPlans = plans.filter(p => !p.is_free && p.price_monthly && (cycle === 'quarterly' ? p.price_quarterly : p.price_yearly))
    if (paidPlans.length === 0) return 0
    
    const avgMonthly = paidPlans.reduce((sum, p) => sum + (p.price_monthly || 0), 0) / paidPlans.length
    const avgCyclePrice = paidPlans.reduce((sum, p) => sum + ((cycle === 'quarterly' ? p.price_quarterly : p.price_yearly) || 0), 0) / paidPlans.length
    const monthlyEquivalent = avgMonthly * (cycle === 'quarterly' ? 3 : 12)
    
    if (monthlyEquivalent === 0) return 0
    return Math.round(((monthlyEquivalent - avgCyclePrice) / monthlyEquivalent) * 100)
  }

  const quarterlyDiscount = calculateDiscount('quarterly')
  const annualDiscount = calculateDiscount('annual')
  
  // Check if quarterly/annual pricing is available
  const hasQuarterlyPricing = plans.some(p => p.price_quarterly && p.price_quarterly > 0)
  const hasAnnualPricing = plans.some(p => p.price_yearly && p.price_yearly > 0)
  ...
  const transformedPlans: TransformedPlan[] = plans.map((plan) => {
    ...
    return {
      ...
      quarterlyPrice: plan.price_quarterly || 0,
    ...
    <>
      <div className="space-y-8 lg:space-y-12">
        <div className="space-y-4">
          ...
          {(hasQuarterlyPricing || hasAnnualPricing) && (
              ...
              {transformedPlans.map((plan) => {
                const displayPrice =
                  plan.price === "Custom" 
                    ...
                    ? plan.quarterlyPrice
                    : plan.monthlyPrice
                const billingPeriod = plan.price === "Custom" ? "" : billingCycle === "annual" ? "/year" : billingCycle === "quarterly" ? "/quarter" : "/month"
